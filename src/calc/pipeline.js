// The Optii-Savr duty/GST/NPV calculation engine and PDF report generator.
// Reads the wizard's form fields (getAllInputValues), runs the per-scheme
// net-benefit calculations, picks the matching report template, and stamps
// the computed values onto it via pdf-lib. This module still relies on a
// large block of shared mutable state (below) rather than threading every
// value as an explicit parameter -- that tight coupling is inherited as-is
// from the pre-rebuild implementation; splitting it further into per-scheme
// modules is deferred to a follow-up pass once Phase 3's tests exist as a
// safety net; see RUNBOOK.md.
import { reportFatal } from "../shared/errors.js";
import {
  getSafeFloatInput,
  formatNumberPDF,
  processValues,
...
export async function getAllInputValues() {
  ...
  let NetBeniftForAA =
    ...
    normalizeValue(lastNPVrodtep*40/100);
  ...
  let NetBeniftForEOU =
    ...
    normalizeValue(EOUValue == "N/A" ? 0 : lastNPVrodtep*40/100);
...
async function updatePDFAndDownloadImpl(
  ...
  function getSafeFormatted(index, fallbackCheckIndex) {
    return allValues[fallbackCheckIndex] === "N/A"
      ? "N/A"
